...
class PageDetail(object):
    def __init__(self):
        ...
        self.sheet.write(0, 7, '分类号', self.basic_style)
        self.sheet.write(0, 8, '发表时间', self.basic_style)
        self.sheet.write(0, 9, '数据库', self.basic_style)
    ...
    def get_detail_page(self, session, result_url, page_url,
        ...
        get_res = self.session.get(page_url, headers=HEADER).url
        self.pars_page(get_res)
    ...
    def pars_page(self, detail_page):
        ...
        self.ClassificationNumber = ''

        res =requests.get(detail_page)
        dom = etree.HTML(res.text)
        if dom.xpath("//li[@class='top-space'][3]/p//text()"):
            ClassificationNumber = dom.xpath("//li[@class='top-space'][3]/p//text()")[0]
            self.ClassificationNumber = ClassificationNumber

        else:
            self.ClassificationNumber = '无分类号'
    ...
    def create_list(self):
        ...
        self.reference_list.append(self.ClassificationNumber)  # 分类号
