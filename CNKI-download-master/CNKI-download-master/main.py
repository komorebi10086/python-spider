...
# ip 登录
IP_LOGINURL = 'https://kns.cnki.net/KNS8/login/LoginIn'
...
class SearchTools(object):
    ...
    def search_reference(self, ueser_input):
        ...
        key_value_unit = quote(ueser_input.get('au_1_value2'))
        self.get_result_url = GET_PAGE_URL + first_post_res.text + '&t=1544249384932&keyValue=' + key_value_unit + '&S=1&sorttype='
    ...
    def parse_page(self, download_page_left, page_source):
        ...
        for index, tr_info in enumerate(tr_table.find_all(name='tr')):
            ...
            for idx, td_info in enumerate(tr_info.find_all(name='td')):
                ...
                # dt_url = td_info.find('a', attrs={'class': 'KnowledgeNetLink'})
                # 排除不是所需要的列
                if dt_url:
